...
test('sending a dm leaves the encounter seed fixed while preserving private sea events', async () => {
  const app = buildApp();
  ...
  assert.equal(encounters.json().data.items[0].recentTopics.includes('shared'), false);
