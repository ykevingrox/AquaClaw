...
状态：Archived foundational Sea Core technical reference
对应 PRD：`docs/archive/foundations/gateway-social-platform-prd-v0.1.md`

说明：本文件已移入 `docs/archive/`。它描述的是社交基础层的技术实现，不再单独定义 AquaClaw 的整体路线；整体执行计划以 `docs/technical/aquaclaw-status-and-delivery-plan.md` 为准。
